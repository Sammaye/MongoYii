...
class EMongoDocument extends EMongoModel {
    ...
    public function __construct($scenario = 'insert') {
        ...
        if (!$this->getDbConnection()->getObjCache(get_class($this)) && get_class($this) != 'EMongoDocument' /* We can't cache the model */) {
            ...
            $this->getDbConnection()->setObjectCache(get_class($this), sizeof($virtualFields) > 0 ? $virtualFields : null, sizeof($documentFields) > 0 ? $documentFields : null
            );
        }
    ...
    public function insert($attributes = null) {
        ...
        if ($this->beforeSave()) {
            ...
            if (!isset($this->{$this->primaryKey()}))
                $this->{$this->primaryKey()} = new MongoId;
            if ($this->getCollection(EMongoClient::WRITE)->insert($this->getRawDocument(), $this->getDbConnection()->getDefaultWriteConcern())) {
                $this->afterSave();
    ...
    public function update($attributes = null) {
        ...
        if ($this->beforeSave()) {
            ...
            if ($attributes !== null) {
                ...
                $this->getCollection(EMongoClient::WRITE)->save($this->getAttributes($attributes));
            $this->afterSave();
            return true;
        }
        else
    ...
    public function exists($criteria = array()) {
        $this->trace(__FUNCTION__);
        return $this->getCollection(EMongoClient::READ)->findOne($criteria) !== null;
    }
    ...
    public function equals($record) {
        return $this->collectionName(EMongoClient::READ) === $record->collectionName(EMongoClient::READ) && (string) $this->getPrimaryKey() === (string) $record->getPrimaryKey();
    }
    ...
    public function findOne($criteria = array()) {
        ...
        if ((
                $record = $this->getCollection(EMongoClient::READ)->findOne($this->mergeCriteria(
    ...
    public function deleteByPk($pk, $criteria = array(), $options = array()) {
        ...
        return $this->getCollection(EMongoClient::WRITE)->remove(array_merge(array($this->primaryKey() => $pk), $criteria), array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options));
    }
    ...
    public function updateByPk($pk, $updateDoc = array(), $criteria = array(), $options = array()) {
        ...
        $pk = $this->getMongoId($pk);
        return $this->getCollection(EMongoClient::WRITE)->update($this->mergeCriteria($criteria, array($this->primaryKey() => $pk)), $updateDoc, array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options));
    }
    ...
    public function updateAll($criteria = array(), $updateDoc = array(), $options = array('multi' => true)) {
        $this->trace(__FUNCTION__);
        return $this->getCollection(EMongoClient::WRITE)->update($criteria, $updateDoc, array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options));
    }
    ...
    public function deleteAll($criteria = array(), $options = array()) {
        $this->trace(__FUNCTION__);
        return $this->getCollection(EMongoClient::WRITE)->remove($criteria, array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options));
    }
    ...
    public function refresh() {
        ...
        $this->trace(__FUNCTION__);
        if (!$this->getIsNewRecord() && ($record = $this->getCollection(EMongoClient::READ)->findOne(array($this->primaryKey() => $this->getMongoId($this->getPrimaryKey())))) !== null) {
            $this->clean();
     ...
     */
    public function getCollection($connectionType) {
        return $this->getDbConnection($connectionType)->{$this->collectionName()};
    }
